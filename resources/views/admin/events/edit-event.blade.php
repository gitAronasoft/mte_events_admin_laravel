                ...
                {{ Form::open(array('url' => 'admin/event/update' , 'autocomplete'=>'off', 'enctype' =>'multipart/form-data')) }}
                    ...
                    <div class="form-row mb-4">
                        <div class="form-group">
                            ...
                            @if(count($categories)>0)    
                                @php
                                    $str_arr = explode (",", $eventDetail->event_category_id); 
                                @endphp                      
                                @foreach($categories as $category)                           
                                    <div class="custom-control custom-checkbox w-100">
                                        @if(!empty($eventDetail->event_category_id))
                                            <input type="checkbox" name="category[]" value="{{$category->id}}" class="custom-control-input" {{in_array($category->id, $str_arr) ? 'checked' : ''}} id="checked{{$category->id}}">
                                        @else 
                    ...
                    <div class="form-group">
                        <div class="input-group mb-3">
                            ...
                            <input type="text" class="form-control" required id="autocomplete" name="eventLocation" value="{{$eventDetail->eventLocation}}" aria-label="">
                            <input type="hidden" class="form-control" id="latitude" name="latitude" value="{{$eventDetail->latitude}}" />
                            <input type="hidden" class="form-control" id="longitude" name="longitude" value="{{$eventDetail->longitude}}" />
                        </div>
                    </div>
